...
The example uses Vulkan Memory Allocator, because nobody wants to allocate memory manually.

It also separates the code into modules and functions instead of providing you with a one-billion-lines single file.

The example uses Ash from git, not a release, because the latest Ash release targets prehistoric Vulkan 1.3.

Also, this example bundles the vk-mem crate source code, because it had to be modified to work with Ash from git.
