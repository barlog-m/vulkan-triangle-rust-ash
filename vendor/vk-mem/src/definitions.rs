...
impl<'a> PoolCreateInfo<'a> {
    pub fn push_next<T: vk::Extends<vk::MemoryAllocateInfo<'static>> + vk::TaggedStructure<'a>>(
        &mut self,
        next: &'a mut T,
    ) {
        // Inline equivalent of ash's `TaggedStructure::push()`:
        // prepend `next` at the head of the `pMemoryAllocateNext` pNext chain.
        // Both pointers point to base `pNext` chain structures, so the casts are layout-compatible.
        let next_base = unsafe { &mut *<*mut T>::cast::<vk::BaseOutStructure>(next) };
        assert!(
            next_base.p_next.is_null(),
            "push_next() expects a struct without an existing p_next pointer chain (equal to NULL)"
        );
        next_base.p_next = self.memory_allocate_next as *mut vk::BaseOutStructure<'static>;
        self.memory_allocate_next = next as *const T as *const std::ffi::c_void;
    }
}
...
pub struct AllocationCreateInfo {
    ...
    /// like `vkGetBufferMemoryRequirements2`, which VMA automatically uses and respects.
    /// Extra alignment may be needed in some cases, like when using a buffer for acceleration structure scratch
    /// (`VkPhysicalDeviceAccelerationStructurePropertiesKHR::minAccelerationStructureScratchOffsetAlignment`) or when doing interop with OpenGL.
    pub min_alignment: vk::DeviceSize,
}
...
impl From<&AllocationCreateInfo> for ffi::VmaAllocationCreateInfo {
    fn from(info: &AllocationCreateInfo) -> Self {
        ...
        ffi::VmaAllocationCreateInfo {
            ...
            minAlignment: info.min_alignment,
        }
    }
}
